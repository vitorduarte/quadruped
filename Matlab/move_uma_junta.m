...
if (clientID>-1)
    ...
    [returnCode, junta5]=vrep.simxGetObjectHandle(clientID,'Jmotor3',vrep.simx_opmode_blocking);
    %if(returnCode==0)
    %disp(strcat('Jmotor',int2str(i),' Connected')) 
     [~,present_pos]=vrep.simxGetJointPosition(clientID,junta5,vrep.simx_opmode_streaming)
     %present_pos = 0;
     %while(true)
      [~]=vrep.simxSetJointTargetPosition(clientID,junta5,pos_rad(1),vrep.simx_opmode_oneshot);
      pause(2)
      [~]=vrep.simxSetJointTargetPosition(clientID,junta5,pos_rad(2),vrep.simx_opmode_oneshot);
      pause(2)
